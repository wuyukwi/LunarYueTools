...
namespace core
{
    ...
    private:
        ...
        public:
            explicit cmd_base(const std::string& name,
                              ...
                              bool               dominant,
                              bool               is_variadic) :
                name(name), command(!name.empty() ? "-" + name : ""), alternative(!alternative.empty() ? "--" + alternative : ""),
                description(description), required(required), handled(false), arguments({}), dominant(dominant), variadic(is_variadic)
            ...
            inline bool is(const std::string& given) const { return given == command || given == alternative; }
        };
        ...
        public:
            explicit cmd_function(const std::string& name,
                                  ...
                                  const std::string& description,
                                  bool               required,
                                  bool dominant) : cmd_base(name, alternative, description, required, dominant, argument_count_checker<T>::variadic)
            {}
        ...
        static unsigned int parse(const std::vector<std::string>& elements, const unsigned int& /*unused*/, int numberBase = 0)
        {
            if (elements.size() != 1)
            {
                throw std::bad_cast();
        ...
        static unsigned long parse(const std::vector<std::string>& elements, const unsigned long& /*unused*/, int numberBase = 0)
        {
            if (elements.size() != 1)
            {
                throw std::bad_cast();
    ...
    public:
        explicit parser(int argc, const char** argv) : appname_(argv[0])
        {
        ...
        explicit parser(int argc, char** argv) : appname_(argv[0])
        {
            for (int i = 1; i < argc; ++i)
            {
                arguments_.push_back(argv[i]);
        ...
        void enable_help()
        {
            set_callback("h",
                         "help",
                         std::function<bool(callback_args&)>([this](callback_args& args) {
        ...
        int commands() const { return static_cast<int>(commands_.size()); }

        inline const std::string& app_name() const { return appname_; }
