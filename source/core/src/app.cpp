...
int App::run(std::string&& title, int w, int h, int argc, char* argv[])
{
    ...
    while (!done)
    {
        ...
        if (fb_width > 0 && fb_height > 0 && (g_SwapChainRebuild || g_MainWindowData.Width != fb_width || g_MainWindowData.Height != fb_height))
        {
            ...
            ImGui_ImplVulkanH_CreateOrResizeWindow(
                g_Instance, g_PhysicalDevice, g_Device, &g_MainWindowData, g_QueueFamily, g_Allocator, fb_width, fb_height, g_MinImageCount);
    ...
    auto lastTime = window_.GetDeltaTime();
    while (running_)
    {
        auto  currentTime = window_.GetDeltaTime();
